...
public class BoardFrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//서블릿 요청 시 GET방식 요청이 들어오면 또는 POST방식 요청이 들어오면
		// 공통으로 처리하기 위해 doGet(), doPost() 메서드에서 호출하는 메서드 자동으로 호출되는 메서드
		// => 파라미터로 request객체와 reponse 객체를 전달 받음
		...
		if(command.equals("/BoardWriteForm.bo")) {
			 ...
			 *  - 기존 서블릿 주소가 그대로 유지되어야 하므로 Dispatcher 방식 포워딩
			...
			// BoardWriteProAction 클래스 객체 생성 후 execute() 메서드 호출
			
			// => Action 클래스는 Action 인터페이스를 구현하므로 다형성 활용 
			action = new BoardWriteProAction();
								
			// 2. Action 클래스의 execute() 메서드 호출
			// => 리턴되는 ActionForward 객체 전달 받기 (직접 생성하지 않음!)
			// => throws에 의해 예외가 전달 되므로 try-catch 필요
			try {
			...
			System.out.println("BoardList.bo로 포워딩! 감지해라");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 서블릿 요청시 POST 방식 요청이 들어오면 자동으로 호출되는 메서드
